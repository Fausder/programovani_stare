...
__global__ void thread_hierarchy()
{
    ...
    // Calculate the total width of the grid in terms of threads
    int width = gridDim.x * blockDim.x;
    
    // Calculate the linear index
    int linear_index = l_y * width + l_x;
    
    printf( "Block{%d,%d}[%d,%d] Thread{%d,%d}[%d,%d] [%d,%d] Linear Index: %d\n",
        gridDim.x, gridDim.y, blockIdx.x, blockIdx.y,
        blockDim.x, blockDim.y, threadIdx.x, threadIdx.y, l_x, l_y, linear_index );
}
